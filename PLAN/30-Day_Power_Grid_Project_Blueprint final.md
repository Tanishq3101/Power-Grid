...
---
## 📝 Logging (built, in `utils/logger.py` — never mentioned in the original 30-day plan)

```
This exists in the actual repo but was never part of any day's tasks
in the original blueprint, so it was undocumented until now.

How it works (get_logger(name), imported as
`from utils.logger import get_logger` at the top of a module):
→ Every module gets its own named logger (usually __name__), but
  they all share one format:
  "%(asctime)s | %(levelname)-8s | %(name)s | %(message)s"
→ Writes to TWO places at once: console (stdout) AND a shared file,
  logs/power_grid_marl.log
→ Default level: INFO
→ Safe to call get_logger() multiple times with the same name —
  handlers are cached so you don't get duplicate log lines
→ Log directory path comes from config/constants.py: LOG_DIRECTORY
→ logs/ is correctly gitignored (`logs/` and `*.log` are both in
  .gitignore) — only logs/.gitkeep is tracked, so the folder exists
  for a fresh clone but nobody's actual log noise gets committed

Known gap (found by actually reading the file, not assumed):
→ utils/logger.py's own docstring claims logs go to "a rotating
  file," but the code uses plain logging.FileHandler, not
  RotatingFileHandler — there's no actual size cap or rotation. On a
  long training run (Week 2-3, hundreds of thousands of timesteps)
  power_grid_marl.log can grow unbounded. Fix is small: swap in
  logging.handlers.RotatingFileHandler with a maxBytes/backupCount,
  or at minimum fix the docstring to stop claiming a behavior that
  isn't there. Either way, this belongs in Day 26 (Code Cleanup) —
  added below.
→ logs/power_grid.log (no "_marl") is a stale leftover from an
  earlier version of this file, before the log filename was renamed
  to power_grid_marl.log — nothing in the current code writes to it
  anymore. Safe to delete; it's gitignored either way so it's not
  tracked, but it's dead weight sitting in a teammate's local clone.

Day 26 (Code Cleanup) gets one more task added:
→ Either implement real rotation (RotatingFileHandler) or fix the
  docstring to match what the code actually does — pick one, don't
  leave them contradicting each other
→ Delete the stale logs/power_grid.log locally on each of our 3
  machines (it won't affect anyone else since it was never tracked)
```
...
Project Structure (as originally planned, Day 1):
power_grid_marl/
│
├── env/
...
Project Structure (ACTUAL, as of the CI/testing work — supersedes the
tree above; several folders and files got added along the way that
weren't in the original Day 1 plan, flagged below):

NOTE: your screenshots showed agents/ and config/ expanded, plus the
full top-level listing — that part is confirmed directly. baselines/,
dashboard/, data/, evaluation/, results/, training/, utils/ weren't
expanded in the screenshots; their contents below are inferred from
files we've directly edited/discussed in this conversation (e.g.
baselines/opf_baseline.py, env/grid_env.py), not independently
verified. If training/, dashboard/, or results/ are still empty
(Week 2+ work not reached yet), that's expected — update this tree
once those days are actually built.

power_grid_marl/
│
├── .github/
│ └── workflows/
│   └── ci.yml              ← 6-job CI pipeline (NEW, not in original plan)
│
├── agents/
│ ├── __init__.py
│ ├── dispatch_agent.py
...
│ └── compare.py
│
├── logs/                    ← NEW, not in original plan
│ ├── .gitkeep
│ ├── power_grid.log
│ └── power_grid_marl.log
│
├── PLAN/                    ← NEW, not in original plan
│
├── results/
│ ├── plots/
│ └── models/
...
├── tests/
│ ├── __init__.py
│ ├── conftest.py            ← NEW, sys.path setup for imports
│ ├── test_grid.py
│ ├── test_physics.py
│ └── test_environment.py
│
├── TODOS/                   ← NEW, not in original plan
│
├── training/
│ ├── train_single.py
│ ├── train_marl.py
...
├── CI_SETUP_GUIDE.md        ← NEW, explains the CI pipeline
├── config.yaml              ← NEW, runtime config (bus_count, weights, etc.)
├── LICENSE
├── PROJECT_ROADMAP.md       ← NEW
├── pyproject.toml           ← NEW, black/isort/mypy/pytest/coverage config
├── README.md
├── requirements.txt
└── verify_setup.py          ← NEW, checks all libraries import correctly

Not tracked in git (build/tool artifacts, ignored via .gitignore):
.mypy_cache/, .pytest_cache/, config/__pycache__/, .git/
...
→ Create config file
→ Fix utils/logger.py: implement real log rotation
  (logging.handlers.RotatingFileHandler) or fix the docstring that
  currently claims rotation happens when it doesn't — see 📝 Logging
  section above
→ Delete the stale logs/power_grid.log leftover locally
...
✅ No debug code left
✅ Consistent style
✅ Log rotation actually implemented, or docstring corrected to match
  reality — not both claiming and not-doing rotation
